...
//             BindGroupLayout, BindGroupLayoutId, Shader, ComputePipeline,
...
// #[derive(Debug)]
// pub enum PipelineDescriptor<'s> {
//     RenderPipelineDescriptor(Box<wgpu::RenderPipelineDescriptor<'s>>),
//     ComputePipelineDescriptor(Box<wgpu::ComputePipelineDescriptor<'s>>),
// }
// assert_impl_all!(PipelineDescriptor: Send, Sync);
...
// #[derive(Debug)]
// pub struct CachedPipeline<'s> {
//     pub descriptor: PipelineDescriptor<'s>,
//     pub state: CachedPipelineState,
// }
// assert_impl_all!(CachedPipeline: Send, Sync);
...
// }
// assert_impl_all!(CachedPipelineState: Send, Sync);
...
//     processed_shaders: HashMap<Vec<ShaderDefVal>, Arc<wgpu::ShaderModule>>,
//     dependents: HashSet<Arc<Shader>>,
// }
// assert_impl_all!(ShaderData: Send, Sync);

// #[derive(Default)]
// struct ShaderCache {
//     data: HashMap<Arc<Shader>, ShaderData>,
//     shaders: HashMap<Arc<Shader>, Shader>,
// }
// assert_impl_all!(ShaderCache: Send, Sync);
...
//     UInt(String, u32),
// }
// assert_impl_all!(ShaderDefVal: Send, Sync);
...
// impl ShaderCache {
//     fn get(
//         &mut self, render_device: &RenderDevice, pipeline: CachedPipelineId,
//         shader: &Arc<Shader>, shader_defs: &[ShaderDefVal],
//     ) -> Result<Arc<wgpu::ShaderModule>, PipelineCacheError> {
//         let shader = self.shaders.get(shader)
//             .ok_or_else(|| PipelineCacheError::ShaderNotLoaded(shader.clone_weak()))?;
//         let data = self.data.entry(shader.clone_weak()).or_default();
...
//                 render_device.device.push_error_scope(wgpu::ErrorFilter::Validation);
//                 let shader_module = render_device.create_shader_module(module_descriptor);
//                 let error = render_device.device.pop_error_scope();

//                 // `now_or_never` will return Some if the future is ready and None otherwise.
//                 // On native platforms, wgpu will yield the error immediately while on wasm it may take
//                 // longer since the browser APIs are asynchronous.
...
//     fn clear(&mut self, shader: &Arc<Shader>) -> Vec<CachedPipelineId> {
//         let mut shaders_to_clear = vec![Arc::downgrade(shader)];
...
// type LayoutCacheKey = (Vec<BindGroupLayoutId>, Vec<wgpu::PushConstantRange>);

// #[derive(Default)]
// struct LayoutCache {
//     layouts: HashMap<LayoutCacheKey, Arc<wgpu::PipelineLayout>>,
// }
// assert_impl_all!(LayoutCache: Send, Sync);
...
//     #[error("Pipeline could not be compiled because the following shader is not loaded yet: {0:?}")]
...
// }
// assert_impl_all!(PipelineCacheError: Send, Sync);
...
// impl std::iter::FusedIterator for ErrorSources<'_> {}